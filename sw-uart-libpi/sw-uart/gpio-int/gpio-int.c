...
int gpio_event_detected(unsigned pin) {
  if(pin >= 32)
    return -1;
  if (GET32(GPEDS0) & (1 << pin)) // p.96, event detect status register
    return 1;
  return 0;
}
...
int gpio_event_clear(unsigned pin) {
  if(pin >= 32)
    return -1;
  PUT32(GPEDS0, 1 << pin); // p.96, event detect status register
  return 0;
}
...
int gpio_int_rising_edge(unsigned pin) {
  if(pin >= 32)
    return -1;
  PUT32(GPAREN0, 1 << pin); // p.99, async rising edge detect enable
  return 0;
}
...
int gpio_int_falling_edge(unsigned pin) {
  if(pin >= 32)
    return -1;
  PUT32(GPAFEN0, 1 << pin); // p.100, async falling edge detect enable
  return 0;
}
