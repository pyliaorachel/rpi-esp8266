...
int sw_rpi_putchar(int c) { sw_uart_putc(c); return c; } 

// XXX: check sign promotion.
static int sw_isdigit(int c) { return c >= '0' && c <= '9'; }

void (sw_panic)(const char *file, int lineno, const char *msg) {
	sw_putk("<PANIC>:");
        sw_printk("%s:%d: PANIC PANIC PANIC: %s\n", file, lineno, msg);
        rpi_reboot();
}
#endif
...
static int sw_va_printk(char *buf, int n, const char *fmt, va_list args) {
		...
		if(*fmt != '%')
			...
			*p++ = *fmt;
			...
			case 'c':
				...
				break;
			default: 
			  //sw_panic("printk: not handling specifier '%c'\n", *fmt);
				return 0;   // ugh
			}
			fmt++;
...
int sw_snprintk(char *buf, size_t n, const char *fmt, ...) {
	va_list args;
        va_start(args, fmt);
		int sz = sw_va_printk(buf, n, fmt, args);
	va_end(args);
	return sz;
}

// i think the f-omit-frame-pointer was messing things up?
int sw_printk(const char *fmt, ...) {
	...
	va_list args;
        ...
        va_start(args, fmt);
		int sz = sw_va_printk(buf, sizeof buf, fmt, args);
	va_end(args);

	int i;
	for(i = 0; buf[i]; i++)
		sw_rpi_putchar(buf[i]);
	return sz;
}


// needs a newline, right?
int sw_putk(const char *p) {
	for(; *p; p++)
		sw_rpi_putchar(*p);
	return 1;
}
